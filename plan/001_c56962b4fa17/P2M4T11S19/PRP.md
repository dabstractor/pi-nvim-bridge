---
name: "P2.M4.T11.S19 — init.lua with setup() and default config options"
description: |
  **Bootstrap the entire Neovim plugin tree** by creating `plugin/lua/pi-editor/init.lua`
  for `pi-editor.nvim`. It exposes `M.setup(opts)` that deep-merges a user options table
  over documented PRD §10.5 defaults (via `vim.tbl_deep_extend("force", …)`), stores the
  resolved config on `M.config`, exposes a typed-`nil` `M.bridge` placeholder for PRD §7.7,
  and is annotated with LuaCATS (`---@class`/`---@field`/`---@param`/`---@return`) [Mode A] docs.
  `require("pi-editor").setup({})` must run error-free in a stock Neovim 0.10+ (verified on
  0.12.4) with **zero** external Lua dependencies. This is the FIRST task of P2 — the
  `plugin/` directory does NOT exist yet; this task CREATES it.
  NARROW scope guard — this task does NOT create or wire: `plugin/plugin/pi-editor.lua`
  (the VimEnter shim = **S20**), the activation gate / `PI_EDITOR_BRIDGE` reader (**S21**),
  `plugin/ftplugin/pi-prompt.lua` (**S22**), or `plugin/lua/pi-editor/bridge.lua` (**S24**).
  `setup()` is PURELY config: merge + store + return — side-effect-free (no autocmds, no
  `vim.api` calls, no `require` of other pi-editor modules, no env-var reads). Keeping it
  pure is what makes it unit-testable in isolation here.
  STATUS (planning): every validation command in this PRP was LIVE-VERIFIED against the
  installed Neovim 0.12.4 + plenary.nvim (see `research/live-verification.md`), closing the
  "no shell tool" gap left by the prior `P2M1T1S1` planning pass (renumbered → this task).
---
...
- `plugin/tests/smoke.lua` — plenary-FREE standalone smoke test (the Level-1 gate;
  sourced via `:luafile` because heredocs do NOT work in `-c`/`+` args — GOTCHA #10).
...
**Success Definition** (all assertions below are LIVE-VERIFIED to pass — see Validation):
- `require("pi-editor").setup({})` returns the merged config table and sets `M.config`;
  no error in `nvim --headless --clean -u NORC`.
- `M.config` after `setup({})` deep-equals `M.defaults`.
- After `setup({ debounce_ms = 50, autosave_on_exit = false, menu = { max_height = 40 } })`:
  `config.debounce_ms == 50`, `config.autosave_on_exit == false` (default `true` overridden —
  proves `"force"` is value-based, not truthiness-based), `config.menu == { max_height = 40,
  border = "rounded" }` (sibling default preserved), and `M.defaults` is **unchanged**.
...
**S20** (VimEnter shim), **S21** (activation gate), **S22** (ftplugin), **S24**
(bridge.lua), **S42** (health.lua). This is developer scaffolding, not end-user-facing.

**Use Case**: Establishes the config seam every later P2 module reads
(`require("pi-editor").config`) and the public surface the optional completion-engine
sources (`blink_source.lua`/`cmp_source.lua`, M12) and user code reach through
`require("pi-editor").bridge`. De-risks "can Neovim find & load our module?" before any
real socket/completion logic lands.

**Pain Points Addressed**: Without this module there is nothing for downstream tasks to
build on; getting the option shape, merge semantics, and field names locked NOW (with
typed annotations + tests) means S21/S24/S42 can rely on them verbatim instead of
rediscovering them.
...
- **Faithful to pi's UX contract (PRD §10.5 / §7.5).** The defaults
  ...
  built-in completion menu feel like pi's own provider. Getting them typed and merged
  correctly here means later tasks can rely on them.
- **Pre-positions the discovery seam.** `M.bridge` is the handle that the optional
  completion-engine sources and user code use to issue RPCs (PRD §7.7). Declaring it
  now (as a typed `nil` placeholder) locks the public surface so `bridge.lua` (task S24)
  just has to *populate* it.
- **Integrates with the (complete) extension work.** The bridge extension already writes
  `process.env.PI_EDITOR_BRIDGE` on `session_start` (P1.M3.T8.S16 — DONE; see
  `extension/pi-editor-bridge.ts:startBridge`). The Neovim-side *reader* of that env var
  is the activation gate (task S21) — **NOT this task**. This task only owns the config
  table that S21 will read `M.config.env_var or "PI_EDITOR_BRIDGE"` from.
...
- `M.bridge` — `nil` placeholder (populated by `bridge.lua` later, PRD §7.7).
- `M.setup(opts)` — `opts = opts or {}`; `M.config = vim.tbl_deep_extend("force", M.defaults, opts)`;
  return `M.config`.
- [Mode A] LuaCATS (`---@class`/`---@field`/`---@param`/`---@return`) docstrings so
  `lua-language-server` and `:help` hover show the option docs.
...
this PRP + the verified research notes under `research/` + the exact commands below.
Every API (`vim.tbl_deep_extend`, `vim.opt.runtimepath`, plenary's
`require("plenary.busted").run`) is cited with a source and a **LIVE-VERIFIED** runnable
command (see `research/live-verification.md`). The plugin-root-vs-repo-root `runtimepath`
gotcha (the #1 cause of `module 'pi-editor' not found`) is spelled out in §Known Gotchas
and embedded in every validation command.
...
- url: https://neovim.io/doc/user/lua.html  # :help vim.tbl_deep_extend
  ...
  critical: "'force' is VALUE-based, not truthiness-based — a user 'false' DOES
             override a default 'true'. LIVE-VERIFIED (research/live-verification.md §1).
             Must do 'opts = opts or {}' first or it throws."

- url: https://github.com/neovim/neovim/issues/23654
  why: "Confirms list/array tables are replaced wholesale (not element-merged)."
  critical: "Not a problem here ('menu' is a dict), but matters for any future array
             option — it would be fully replaced, not appended."
...
- file: plan/001_c56962b4fa17/P2M4T11S19/research/live-verification.md
  why: "LIVE-VERIFIED proof that every validation command in this PRP runs green on the
        installed nvim 0.12.4 + plenary, AND proof of the core merge/runtimepath/heredoc
        claims. Closes the 'no shell tool' gap of the prior P2M1T1S1 research."

- file: plan/001_c56962b4fa17/P2M1T1S1/research/setup-pattern.md
  why: "Detailed (source-read) writeup of the canonical setup() structure, force/keep
        semantics, false-overrides-true proof, non-mutation guarantee, and the full
        LuaCATS annotation set (pi-editor.MenuConfig / pi-editor.Config)."
  note: "Its claims are now LIVE-VERIFIED by the file above."

- file: plan/001_c56962b4fa17/P2M1T1S1/research/testing.md
  why: "Detailed (source-read) writeup of headless smoke testing + plenary harness,
        including the CRITICAL runtimepath gotcha (plugin/ subdir, not repo root)."
  note: "Its commands are now LIVE-VERIFIED green by research/live-verification.md."

- file: plan/001_c56962b4fa17/architecture/external_deps.md
  why: "§1 confirms Neovim 0.12 APIs (vim.uv, vim.json, vim.api) used by LATER modules;
        §6 confirms plenary.nvim + selene/stylua as the plugin test/lint stack. This task
        touches none of those APIs — it is pure Lua tables — but the version floor
        (Neovim 0.10+) and the module path 'lua/pi-editor/' are pinned here."

- file: extension/pi-editor-bridge.ts
  why: "The COMPLETE pi extension that WRITES process.env.PI_EDITOR_BRIDGE (in
        startBridge()). Confirms the env-var name is 'PI_EDITOR_BRIDGE' and the
        descriptor is single-line JSON — which is why the activation gate (S21, not this
        task) will read M.config.env_var or 'PI_EDITOR_BRIDGE'."
...
- docfile: plan/001_c56962b4fa17/prd_snapshot.md
  ...
  why: "These four PRD sections ARE the source of truth for this task's option set and
        the M.bridge exposure requirement (reproduced verbatim in <selected_prd_content>)."
```
...
└── plan/001_c56962b4fa17/
    ...
    ├── P2M1T1S1/{PRP.md, research/{setup-pattern,testing}.md}   # prior pass for THIS task
    └── P2M4T11S19/{PRP.md, research/live-verification.md}       # THIS task (canonical name)
...
> TypeScript extension (`extension/`) and the Neovim plugin (`plugin/`). The `plugin/`
> directory is what a user installs / symlinks onto `runtimepath` (PRD §9.2 calls this
> root `pi-editor.nvim/`). Everything under `plugin/` mirrors a normal standalone plugin
> repo (`lua/`, `plugin/`, `ftplugin/`, `doc/`). Later tasks add
> `plugin/plugin/pi-editor.lua` (S20 VimEnter shim), `plugin/ftplugin/pi-prompt.lua`
> (S22), `plugin/lua/pi-editor/bridge.lua` (S24), etc.
...
-- LIVE-VERIFIED: repo-root require FAILS, plugin-subdir require ok=true.
...
-- A user's autosave_on_exit = false DOES override the default true. LIVE-VERIFIED (a="false").
-- (Good — that's the whole point of a boolean option. But it also means you cannot 'unset'
--  an option by passing false; omit it instead.)

-- GOTCHA #4 — list/array tables are OPAQUE (replaced wholesale, NOT element-merged).
-- (neovim/neovim#23654.) Harmless here because 'menu' is a dict — but if a future option is
-- an array, deep_extend will REPLACE it, not append. Use merge accordingly.

-- GOTCHA #5 — vim.tbl_deep_extend does NOT mutate its arguments; it returns a NEW table.
-- So M.defaults stays pristine. LIVE-VERIFIED (defd=25, defmh=12 after an override setup).
-- (Still asserted in the spec to pin this.)
...
-- So setup({ engine = nil }) keeps the default 'builtin' (the nil key is absent from the
-- table). This is correct/desired; just don't rely on nil to 'mean something'.
...
--   (a) nvim --headless smoke test, (b) plenary spec. Both run with no extra install and
--   are LIVE-VERIFIED green. (Optional: 'cargo install stylua' + 'cargo install selene'.)
...
-- setup() is PURELY config: merge + store + return. Keeping it side-effect-free is what
-- makes it unit-testable in isolation here.

-- GOTCHA #10 — a ':lua <<HEREDOC' does NOT work inside -c / + command-line args.
-- LIVE-VERIFIED on nvim 0.12.4: `nvim ... +"lua <<LUA ... LUA"` → E5107 'unexpected symbol
-- near <'. (The <<HEREDOC syntax only works in a sourced file / interactive typing.) For
-- multi-statement validation from the CLI, either (a) write a script and use `:luafile`, or
-- (b) pass a single -c/+ arg with semicolon-separated statements. That is why
-- plugin/tests/smoke.lua exists and is sourced via :luafile.
```
...
Define these `---@class` types at the top of `init.lua` (before `local M`). They are the
single source of truth for the option shape and give `lua-language-server` hover docs +
completion. (Verified syntax: LuaLS Annotations wiki.)
...
> `env_var` is optional (`?`) and is **deliberately NOT in `M.defaults`** — it has no useful
> default *value* to merge (the activation gate reads it as `M.config.env_var or
> "PI_EDITOR_BRIDGE"`). Keeping it out of `defaults` keeps the defaults table to exactly the
> 5 PRD §10.5 options.
...
Task 2: CREATE plugin/tests/minimal_init.lua  (plenary harness bootstrap)
  - CONTENT (exact — see Implementation Patterns below): compute plugin_root from this
        ...
        plenary, append plugin_root to runtimepath.
  - WHY: makes the in-process `require("plenary.busted").run(...)` style work headlessly.
...
Task 3: CREATE plugin/tests/init_spec.lua  (plenary/busted spec)
  ...
  - ASSERTIONS: assert.are.same (DEEP, for tables/config), assert.are.equals (shallow,
        ...
        assert.has_no.errors (for setup(nil)). (Luassert semantics: research/testing.md §3.)
...
Task 4: CREATE plugin/tests/smoke.lua  (plenary-FREE fast smoke test — the Level-1 gate)
  ...
  - WHY: instant, dependency-free feedback (no plenary). init_spec.lua remains the formal suite.
  - GOTCHA: do NOT inline this as a `:lua <<HEREDOC` in a -c/+ arg — nvim rejects that
        with E5107 (GOTCHA #10, LIVE-VERIFIED). Source it via `:luafile`.
...
> on the installed Neovim 0.12.4 + plenary.nvim (see `research/live-verification.md`).
...
# LIVE-VERIFIED: prints SMOKE_PASS, exit=0.
```
...
echo "exit=$?"
cd ..
# LIVE-VERIFIED: exit=0, prints "Success: 13  Failed: 0  Errors: 0" (13 'it' blocks).
```
...
nvim --headless --clean -u NORC +"lua vim.opt.runtimepath:append('$PLUGIN_ROOT')" \
  +'lua local ok=pcall(require,"pi-editor"); io.stdout:write("require ok: "..tostring(ok).."\n")' +qa 2>&1 | tail -1
# LIVE-VERIFIED: "require ok: true"  (the require succeeds)

echo "-- runtimepath = repo root (WRONG) --"
nvim --headless --clean -u NORC +"lua vim.opt.runtimepath:append('$(pwd)')" \
  +'lua local ok=pcall(require,"pi-editor"); io.stdout:write("require ok: "..tostring(ok).."\n")' +qa 2>&1 | tail -1
# LIVE-VERIFIED: "require ok: false"  (proves the plugin/ subdir is the required entry)

# 3b. A realistic user-config simulation: rtp + setup() in a -u NORC session.
nvim --headless --clean -u NORC +"lua vim.opt.runtimepath:append('$PLUGIN_ROOT')" \
  +'lua require("pi-editor").setup({ menu = { border = "double" } })' \
  +'lua io.stdout:write("engine: "..require("pi-editor").config.engine.." border: "..require("pi-editor").config.menu.border.."\n")' \
  +qa 2>&1 | tail -1
...
- ❌ Don't point `runtimepath` at the repo root — it must be the `plugin/` SUBDIRECTORY
  (GOTCHA #1, LIVE-VERIFIED) or `require("pi-editor")` fails with "module not found".
