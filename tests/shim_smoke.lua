-- === tests/shim_smoke.lua — standalone (plenary-FREE) smoke test for the shim ===
-- Run from the REPO ROOT:
--   nvim --headless --clean -u NORC +"luafile tests/shim_smoke.lua" +qa ; echo exit=$?
-- Exits 0 on pass (prints SMOKE_PASS), 1 on any check failure (via cquit). Zero deps.
-- NOTE: this file is sourced at step 17 (+), AFTER step-12 auto-source of plugin/pi-bridge.lua
...
local plugin_root = vim.fn.fnamemodify(me, ":h:h") -- .../<repo-root>  (rtp entry — GOTCHA #1)
...
vim.cmd("runtime plugin/pi-bridge.lua")
...
local function resource() vim.cmd("runtime plugin/pi-bridge.lua") end

-- CHECK 1: exactly 1 VimEnter autocmd, once=true, group=pi-bridge, callback is a fn.
local a = autovims()
check(#a == 1, "expected exactly 1 VimEnter autocmd, got " .. #a)
if a[1] then
  check(a[1].once == true, "autocmd.once should be true")
  check(a[1].group_name == "pi-bridge", "group_name should be 'pi-bridge', got " .. tostring(a[1].group_name))
...
package.loaded["pi-bridge"] = nil; local pi = require("pi-bridge")
vim.g.pi_calls = 0
...
resource()                                               -- fresh once-autocmd
package.loaded["pi-bridge"] = nil; require("pi-bridge")  -- fresh module, activate == nil
