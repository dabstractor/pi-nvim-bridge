-- === tests/blink_source_spec.lua — plenary/busted spec (the Level-2 gate) ===
...
--- completion_spec.lua's fake_bridge). request() stores the cb (returns a fresh numeric-
--- string id); the spec fires cbs via fake.resolve(idx, err, result) OR
--- fake.resolve_last(err, result). cancel() records the cancelled id (and ALSO fires the
--- cb with "cancelled" — mirroring the real bridge so the ctx.id-guard path is exercised).
...
--- Reset between cases: clear the fake bridge + reset the blink source's supersession
...
--- Build a minimal blink ctx subset the source reads (`bufnr`, `id`, `bounds`, `line`).
--- (The source uses ctx.bufnr + ctx.id; bounds/line are forwarded in case future tasks
--- need them, but the current mapper derives the range from cursor + coords.)
local function make_ctx(bufnr, id)
  return {
    bufnr = bufnr,
    id = id or 1,
    bounds = { line_number = 1, start_col = 0, length = 0 },
    line = "",
    trigger = { character = "/" },
  }
end
...
describe("pi-bridge.blink_source", function()
  ...
  it("new() returns a source with get_trigger_characters/enabled/get_completions/execute as functions", function()
    local src = blink.new()
    assert.is_not_nil(src)
    assert.are.equals("function", type(src.get_trigger_characters))
    assert.are.equals("function", type(src.enabled))
    assert.are.equals("function", type(src.get_completions))
  ...
  -- (4) get_completions happy path: issues getSuggestions with EXACT params + maps items + callback shape
  it("get_completions issues getSuggestions {lines,cursorLine,cursorCol,force=false} + maps items + callback shape", function()
    ...
    local got
    src:get_completions(make_ctx(buf, 1), function(resp) got = resp end)
    ...
    -- resolve with a slash item → mapped to a blink item with label/kind/detail/textEdit/data
    ...
    assert.are.equals(0, it.textEdit.range.start.character) -- end(3) - utf16_len("/mo")=2 → 0... wait 3-3=0
  ...
  -- (5) supersession: a newer ctx.id wins; an older cb is dropped (no callback, no stale items)
  it("supersedes via cancel(prev_id) AND drops a stale response at the ctx.id-guard", function()
    ...
    -- 1st get_completions (ctx.id=1, slow; do not resolve)
    local got1
    src:get_completions(make_ctx(buf, 1), function(resp) got1 = resp end)
    wait_for(200, function() return #fake.requests >= 1 end)
    local id1 = fake.requests[1].id
    local stale_cb = fake.requests[1].cb
    -- 2nd get_completions (ctx.id=2) → cancels request 1 (layer 1) + bumps current_id (layer 2)
    local got2
    src:get_completions(make_ctx(buf, 2), function(resp) got2 = resp end)
    ...
    -- layer 2: resolve the STALE (1st) cb with a result — got1 must NOT fire (no stale items)
    ...
    assert.is_nil(got1, "a stale response must NOT fire callback (ctx.id-guard)")
    -- now resolve the 2nd (current ctx.id) cb → got2 fires
  ...
  describe("bridge nil / disconnected → callback() nil", function()
    it("pi.bridge == nil → callback() nil, no throw", function()
      ...
      assert.has_no.errors(function()
        src:get_completions(make_ctx(0, 1), function(resp) got = resp end)
      end)
  ...
  -- (9) execute happy path: applyCompletion params + callback() immediate + cb set_lines/set_cursor
  describe("execute", function()
    -- helper: drive get_completions to capture a populated blink item with a snapshot
    ...
    -- (9a) execute issues applyCompletion with the EXACT params shape (snapshot)
    it("execute issues applyCompletion with {lines,cursorLine,cursorCol,item,prefix} from the snapshot", function()
      ...
      assert.is_not_nil(item, "must have a populated blink item")
      local n0 = #fake.requests
      local cb_called = false
      src:execute(make_ctx(buf, 1), item, function() cb_called = true end, function() end)
      -- callback() IMMEDIATE (responsive; never hangs blink)
      assert.is_true(cb_called, "callback() must fire IMMEDIATELY (before the RPC resolves)")
    ...
    it("cb success replaces the buffer wholesale + sets cursor (NO -1)", function()
      ...
      src:execute(make_ctx(buf, 1), item, function() end, function() end)
    ...
    -- (9e) malformed item.data → callback() + no throw
    it("malformed item.data (no .pi) → callback() + no throw", function()
      local src = blink.new()
      local cb_called = false
      assert.has_no.errors(function()
        src:execute(make_ctx(0, 1), { data = nil }, function() cb_called = true end, function() end)
        src:execute(make_ctx(0, 1), { data = { prefix = "x" } }, function() cb_called = true end, function() end)
        src:execute(make_ctx(0, 1), nil, function() cb_called = true end, function() end)
      end)
      assert.is_true(cb_called, "callback() must fire on malformed data")
    end)

    -- (9f) bridge nil/disconnected → callback() (buffer left as blink's textEdit)
    it("bridge nil/disconnected → callback() (graceful)", function()
      local src = blink.new()
      pi.bridge = nil
      local cb_called = false
      src:execute(make_ctx(0, 1), { data = { pi = { value = "x" } } }, function() cb_called = true end, function() end)
      assert.is_true(cb_called, "callback() must fire when bridge is nil")
      local fake = fake_bridge({ connected = false })
      pi.bridge = fake
      cb_called = false
      src:execute(make_ctx(0, 1), { data = { pi = { value = "x" } } }, function() cb_called = true end, function() end)
      assert.is_true(cb_called, "callback() must fire when disconnected")
      assert.are.equals(0, #fake.requests, "disconnected bridge must issue no request")
    end)

    -- (9g) does NOT call default_implementation
    it("does NOT call default_implementation (we overwrite wholesale)", function()
      local fake, buf, _win, src, item = populated_item("/mo", 3, { value = "/model", label = "model" }, "/mo")
      local di_called = false
      src:execute(make_ctx(buf, 1), item, function() end, function() di_called = true end)
      wait_for(100, function() return false end)
      assert.is_false(di_called, "default_implementation must NOT be called")
      vim.api.nvim_buf_delete(buf, { force = true })
    end)
  end)

  -- (10) never requires blink.cmp at runtime: package.loaded["blink.cmp"] stays nil
  it("never requires blink.cmp at runtime (package.loaded['blink.cmp'] stays nil)", function()
    -- the module was already required at the top of this spec
    assert.is_nil(package.loaded["blink.cmp"], "package.loaded['blink.cmp'] must be nil (dormant rule)")
  end)

  -- (11) never-throws on bad args to get_completions / execute
  describe("never-throws on bad args", function()
    it("get_completions with a wiped buf → callback() nil, no throw", function()
      local fake = fake_bridge()
      pi.bridge = fake
      local src = blink.new()
    ...
    it("get_completions with nil ctx → callback() nil, no throw", function()
      local fake = fake_bridge()
      pi.bridge = fake
      local src = blink.new()
      local got = "UNSET"
      assert.has_no.errors(function()
        src:get_completions(nil, function(resp) got = resp end)
      end)
      assert.is_nil(got)
    end)
  end)
