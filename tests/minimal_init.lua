-- === tests/minimal_init.lua — plenary harness bootstrap ===
-- Run (from the repo root, OR pass absolute spec path — see Validation):
--   nvim --headless --clean -u tests/minimal_init.lua \
--     -c 'lua require("plenary.busted").run("tests/init_spec.lua")'
local me = debug.getinfo(1, "S").source:sub(2)            -- .../tests/minimal_init.lua
me = vim.fn.fnamemodify(me, ":p")                          -- absolute (relative-path safety)
local plugin_root = vim.fn.fnamemodify(me, ":h:h")         -- .../<repo-root>   (runtimepath entry — GOTCHA #1)
