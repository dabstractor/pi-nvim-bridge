...
test("UNIT: provider-not-captured → rethrows the plain Error (NOT a BridgeRpcError; -32603 safety net)", () => {
	...
	const state = { handshakeComplete: true };
	// S12 does NOT wrap this — it lets the plain Error propagate to handleLine's -32603 net.
	// (S15 will later refine this into a specific code; S12 keeps it flowing — keeps pi safe.)
	assert.throws(
		() =>
			handler(
				{ lines: ["/m"], cursorLine: 0, cursorCol: 2, item: { value: "model", label: "model" }, prefix: "/m" },
