...
`pi-bridge.nvim` renders pi's **live** autocomplete — slash commands (`/model`,
...
- **`pi-nvim-bridge`** — a pi **extension** (TypeScript) that captures pi's
  live `AutocompleteProvider` and serves it over a local Unix socket.
  ([installed separately](../README.md) via `pi install`).
- **`pi-bridge.nvim`** *(this plugin)* — a Neovim (Lua) plugin that activates
...
> `PI_NVIM_BRIDGE` environment variable set (see
> [The `PI_NVIM_BRIDGE` environment variable](#the-pi_nvim_bridge-environment-variable)).
...
- **Neovim >= 0.11** *(not 0.10)* — the exact-UTF-16 cursor conversion in
  ...
  `:checkhealth pi-bridge` enforces this floor.
- The **`pi-nvim-bridge`** pi extension installed and enabled (`pi list`
  shows `pi-nvim-bridge`). It writes `PI_NVIM_BRIDGE` when pi starts an
  editor.
...
> your config). The startup shim (`plugin/pi-bridge.lua`) auto-sources on
> VimEnter.
...
**Defaults** (mirror `lua/pi-bridge/init.lua` `M.defaults` byte-for-byte):
...
| `env_var`          | `"PI_NVIM_BRIDGE"`| Override the bridge-descriptor env var name (read from `config.env_var`, defaulting to `PI_NVIM_BRIDGE` when unset). |

`setup()` returns the resolved config and also stores it as
`require("pi-bridge").config`. The seven keys above are the full public
...
`lua/pi-bridge/blink_source.lua`):
...
```lua
{
  ...
  opts = {
    sources = {
      default = { "pi" },
      providers = {
        pi = { name = "pi", module = "pi-bridge.blink_source" },
      },
    },
  },
}
...
require("cmp").register_source("pi", require("pi-bridge.cmp_source").new())
```
...
…or just run `:checkhealth pi-bridge` (see [Health & diagnostics](#health--diagnostics)).
...
```vim
:checkhealth pi-bridge   " 4 sections: version / env / connection / fd — never throws
:messages                " the one-time "completion unavailable" notify (connect refused / bad token / timeout)
:help pi-bridge          " this plugin's vimdoc (after :helptags)
```

A missing `PI_NVIM_BRIDGE` is reported as **INFO "dormant"** (the expected
...
`:checkhealth pi-bridge` — it will say "dormant" if the env var is unset.

**Q: "Completion doesn't appear when pi opens nvim."**
**A:** Check, in order: (1) the bridge extension loaded — `pi list` shows
`pi-nvim-bridge`; (2) `EDITOR`/`VISUAL`/`externalEditor` is `nvim`; (3) this
...
`$PATH` — see `:checkhealth pi-bridge`.
...
- **Vimdoc:** `:help pi-bridge` ([`doc/pi-bridge.txt`](doc/pi-bridge.txt)).
