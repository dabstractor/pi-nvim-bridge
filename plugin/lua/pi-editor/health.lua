...
--   `:checkhealth pi-editor` discovers this file via a runtimepath glob and runs the
--   literal expression `require("pi-editor.health").check()` (health.lua:152). So `check`
...
--
-- DORMANT ≠ ERROR (PRD §7.1/§11): in a normal nvim session `PI_EDITOR_BRIDGE` is UNSET —
...
--
-- TEST-FRIENDLY: read `vim.health` + `require("pi-editor")`/`.config`/`.descriptor` +
-- `require("pi-editor.bridge")`/`.version`/`.is_connected()`/`.server_info` INSIDE
...
-- is the SERVER's resolution (pi agent bin dir FIRST, then $PATH — pi-editor-bridge.ts:327);
...
--- The `:checkhealth pi-editor` report. Run by the loader as
--- `require("pi-editor.health").check()` (VERIFIED runtime/lua/vim/health.lua:152).
...
function M.check()
  ...
  if pver then
    health.ok(("pi-editor.nvim v%s"):format(tostring(pver)))
  else
    health.warn("could not read pi-editor version (the bridge module failed to load)")
  end
  ...
  if raw == nil and desc == nil then
    health.info(
      ("%s is not set — pi-editor is dormant (this is normal outside a pi editor session)."):format(env_name)
    )
  else
